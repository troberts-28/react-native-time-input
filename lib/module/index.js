...
export default function TimeInput({
  ...
  const [currentLocaleTime] = useState(getLocaleTimeString(null, hideHours, includeSeconds));
  const [currentLocaleTimeParsed] = useState(parseLocaleTimeString(currentLocaleTime));
  const [initialTimeParsed] = useState(() => !initialTime ? null : parseLocaleTimeString(getLocaleTimeString(initialTime, hideHours, includeSeconds)));
